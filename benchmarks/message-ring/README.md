...
| **Go** | goroutines + unbuffered channels | `GOMAXPROCS=1` `GOGC=off` `GODEBUG=asyncpreemptoff=1` | The prototype measured clean at 0.001-0.002% jitter. `asyncpreemptoff=1` prevents the signal-based preemption timer from interrupting a channel-blocked goroutine mid-count. |
