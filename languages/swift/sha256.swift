...
func sha256Block(_ b: [UInt8], _ h: inout [UInt32]) {
...
func sha256_32(_ digest: inout [UInt8]) {
    var b = [UInt8](repeating: 0, count: 64)
    for i in 0..<32 { b[i] = digest[i] }
    b[32] = 0x80
...
for _ in 0..<N { sha256_32(&d) }
var h: Int = 0
for i in 0..<32 { h = (h * 31 + Int(d[i])) % P }
print(h)
print("sha256(\(N))")
