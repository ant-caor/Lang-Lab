...
// final 32-byte digest. All integer; the 32-bit wraparound is explicit (uint: += wraps, >> logical).
class Sha256
{
    ...
    static readonly uint[] H0 = {
        0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19};

    static uint Rotr(uint x, int n) { return (x >> n) | (x << (32 - n)); }

    static void Sha256Block(byte[] b, uint[] h)
    {
        uint[] w = new uint[64];
        for (int i = 0; i < 16; i++)
            w[i] = ((uint)b[i*4] << 24) | ((uint)b[i*4+1] << 16) | ((uint)b[i*4+2] << 8) | b[i*4+3];
        for (int i = 16; i < 64; i++)
        {
    ...
    static void Main(string[] args)
    {
        ...
        byte[] d = new byte[32];
        long s = 42;
        for (int i = 0; i < 32; i++) { s = (s * 1103515245L + 12345L) & 0x7fffffffL; d[i] = (byte)(s % 256); }
        for (int i = 0; i < n; i++) Sha256_32(d);
        long h = 0;
        for (int i = 0; i < 32; i++) h = (h * 31 + d[i]) % P;
        Console.WriteLine(h);
        Console.WriteLine($"sha256({n})");
    }
}
